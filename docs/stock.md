...
| 간밤 ADR | 13 | +11.0%p | −2.2%p | ○ (아래 참고) |
| 전일 삼성전자 | 203 | +0.7%p | −7.3%p | ✗ 효과 없음 |

**ADR을 표본 부족으로 뺐다가 다시 넣었다.** 처음엔 "표본 쌓인 뒤에" 원칙을 적용했는데,
그 원칙은 **이미 측정된 규칙을 인샘플 숫자로 갈아치우지 말라는 뜻**이지 새 재료를 넣지
말라는 뜻이 아니다. ADR은 미국에서 하이닉스 자체를 거래하는 값이라 이론적으로 가장 직접적인
재료이고, 무엇보다 **실전 채점의 컴포넌트별 귀속 분석이 몇 주면 값어치를 알려준다.** 빼두면
영영 모른다. 삼성전자는 이미 어제 한국장에서 같이 움직인 정보라 새로울 게 없다.

임계는 고정값(2)에서 **과반**(`ceil(n/2)`)으로 바꿨다. ADR·나스닥이 수집 실패로 빠질 수
있어 컴포넌트 수가 3~4로 달라지는데, 고정 임계면 재료가 적은 날 신호가 헐거워진다.
재료가 하나뿐이면 "과반"이 그 하나가 돼 단일 지표 신호가 되므로 둘 미만이면 판정하지 않는다.
