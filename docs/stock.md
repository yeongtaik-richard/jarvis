...
- **KRX 휴장일 달력이 없다.** 이게 세 곳에서 새어 나온다: ① `missed` 오탐(휴장일에
  마감 수집이 안 돌면 경고), ② 대상일이 휴장일인 예측이 `expired`로 빠짐(표본 유실),
  ③ 5거래일 지평을 **캘린더 +7일**로 근사하는데 중간에 휴장일이 끼면 실제로는 4거래일이
  된다 — 백테스트는 정확히 5거래일이라 그 주변에서 둘이 다른 걸 잰다. 근본 해결은
  휴장일 달력 도입이고, 그전까지는 광복절·추석 같은 날 근처 표본을 신뢰하지 말 것.
